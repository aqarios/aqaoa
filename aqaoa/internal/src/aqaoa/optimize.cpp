...
lbfgsfloatval_t evaluate(void *instance, const lbfgsfloatval_t *x,
  ...
  aqaoa_instance_t *aqaoa_instance = (aqaoa_instance_t *)instance;
  const int stride = n >> 1;

  inner_gradients_aqaoa(aqaoa_instance->handle, aqaoa_instance->numberOfNodes,
                        aqaoa_instance->depth, aqaoa_instance->polynomialKeys,
                        aqaoa_instance->polynomialValues,
                        aqaoa_instance->polynomialSize, x, x + stride,
                        aqaoa_instance->blockSize, g, g + stride,
                        &expectationValue, aqaoa_instance->rxmethod);
...
static int progress(void *instance, const lbfgsfloatval_t *x,
  ...
  aqaoa_instance_t *aqaoa_instance = (aqaoa_instance_t *)instance;
  aqaoa_instance->iteration = k;
  aqaoa_instance->n_evaluations += ls;

  if (aqaoa_instance->should_log) {
    aqaoa_instance->fx_log[k - 1] = (double)fx;
    for (int i = 0; i < n; ++i) {
      aqaoa_instance->x_log[(k - 1) * n + i] = (double)x[i];
    }
  }
  return 0;
}
...
AqaoaOptimized optimize_aqaoa(
    AqAOAHandle *handle, size_t numberOfNodes, size_t depth,
  ...
  aqaoa_instance_t instance = {handle,         numberOfNodes,    depth,
