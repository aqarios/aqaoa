...
impl AqAOA {
    ...
    fn new(
        ...
        let inner = AqAOAInner::new(&adj, param, block_size, rxmethod);
        Ok(AqAOA { inner })
    }
    ...
    fn new_from_map(
        ...
        let param = build_parameterization(depth, parameterization_method, parameters, time, seed)?;
        let inner = AqAOAInner::new_from_map(num_qubits, &dictionary, param, block_size, rxmethod);
        Ok(AqAOA { inner })
    }
...
pub fn expectation_value(
    ...
    <AqAOAInner as AOAAssociatedFunctions>::expectation_value(
        handle.get(),
        num_qubits,
        depth,
        &polynomial.into(),
...
pub fn optimize(
    ...
    let optresult = <AqAOAInner as AOAAssociatedFunctions>::optimize(
        handle.get(),
        num_qubits,
        depth,
        &polynomial.into(),
